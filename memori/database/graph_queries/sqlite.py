"""
SQLite Graph Query Builder
Uses simple CTEs and iterative queries
"""
...
class SQLiteGraphQueryBuilder(GraphQueryBuilder):
    """SQLite-specific graph query builder"""
    ...
    def build_entity_search_query(
        ...
        entity_placeholders = ",".join(["?" for _ in normalized_entities])
    ...
    def build_graph_expansion_query(
        ...
        """1-hop expansion for SQLite (multi-hop handled iteratively)"""

        seed_placeholders = ",".join(["?" for _ in seed_memory_ids])
    ...
    def build_entity_cluster_query(
        ...
        WHERE me.namespace = ?
            AND me.normalized_value IN ({entity_placeholders})
        GROUP BY me.memory_id, me.memory_type
        HAVING COUNT(DISTINCT me.normalized_value) >= ?
    ...
    def build_relationship_discovery_query(
        ...
        SELECT
            ...
            CASE
                WHEN mr.source_memory_id = ? THEN mr.target_memory_id
                ELSE mr.source_memory_id
    ...
    def build_path_finding_query(
        ...
        """Simple 1-hop path check for SQLite"""
    ...
    def build_shared_entities_query(
        ...
        HAVING COUNT(DISTINCT me2.normalized_value) >= ?
    ...
    def supports_recursive_cte(self) -> bool:
        """SQLite supports recursive CTEs but we use simple iterative approach"""
        return False
